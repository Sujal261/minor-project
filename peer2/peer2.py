...
def train(args):
	...
	features, labels = create_data(args.rank)
	print(f"[rank {args.rank}] training on {args.peer_ip}; parameters={sum(p.numel() for p in model.parameters())}")
	...
	for epoch in range(EPOCHS):
		...
		for start in range(0, LOCAL_SAMPLES, BATCH_SIZE):
			...
			loss.backward()
			averaged = ring_allreduce(flatten_gradients(model), args.peer_ip, args.lport, args.rank, args.ring_size, args.next_addr)
			restore_gradients(model, averaged)
			optimizer.step()
			total_loss += loss.item()
		if epoch == 0 or (epoch + 1) % 3 == 0 or epoch == EPOCHS - 1:
			print(f"[rank {args.rank}] epoch={epoch + 1:02d} loss={total_loss / (LOCAL_SAMPLES / BATCH_SIZE):.4f}")
	...
	print(f"[rank {args.rank}] final_accuracy={accuracy:.3f} model_checksum={checksum:.6f}")
...
def control_listener(args, lock):
	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as listener:
		listener.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
		...
		while True:
			...
			try:
				...
				if ring_size >= 2 and lock.acquire(blocking=False):
					run_args = argparse.Namespace(**vars(args), rank=rank, ring_size=ring_size, next_addr=next_addr)
					threading.Thread(target=training_round, args=(lock, run_args), daemon=True).start()
			finally:
				connection.close()
